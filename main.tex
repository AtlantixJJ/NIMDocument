...
\begin{figure}[H]
  ...
  \caption{\kai 神经图像编辑在动漫人脸上的编辑效果展示，共六组，每组三幅图。每组中，左中右分别为原图、原画加上简笔画图、结果图。从上到下的编辑方式分别是更换发色、更换瞳色、更换发型。}
  \label{fig:demo_face}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\kai 神经图像编辑在鞋子上的编辑效果展示，格式同上图。从上到下分别是更换鞋子颜色或光泽、编辑鞋子形状、基于整体草图的编辑。}
  \label{fig:demo_shoe}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\kai 神经图像编辑在手提包图片上的编辑效果展示，格式同上图。第一行左边是改变颜色，右边是添加光泽，第二行是调整手提包大小，第三行是修改手提包带子形状。}
  \label{fig:demo_hangbag}
\end{figure}
