...
我们经过实践总结，与导师讨论并提出了创新的网络结构与训练方案。当前方案中，简笔画未出现在训练过程中，导致一些编辑不符合预期的情况。我们通过将生成网络分为两个阶段，将简笔画整合入网络训练的过程。流程如下图所示


第一阶段的生成接受随机数输入，第二阶段的生成接受第一阶段的结果与一个草图，输出编辑以后的图片。该图片需要保证编辑距离尽可能小，同时进入判别器进行GAN训练。
...
神经图像编辑项目致力于通过简笔画生成或者编辑一个图像，针对第一类基于神经网络的图像编辑方法中质量瓶颈问题进行了解决方法的探索，贡献如下：
...
接下来我们主要的发展方向有三点，一是前端界面的改进，二是应用下一代训练技术进一步提高生成质量与编辑质量，三是扩展支持的编辑领域，支持场景编辑

\begin{enumerate}
  \item 用户端的改进。丰富网页端的功能，整合入多种模型
\end{enumerate}
